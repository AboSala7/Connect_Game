...
public class MainActivity extends AppCompatActivity {
    ...
    public void drop (View view){
        ...
        for (int[]win : winingPos) {
            if ((places[win[0]] == places[win[1]]) && (places[win[0]] == places[win[2]]) && places[win[0]] != 2) {
               Toast.makeText(getApplicationContext(), "there is a winner", Toast.LENGTH_LONG).show();
                la =  (LinearLayout) findViewById(R.id.l);
                la.setVisibility(View.VISIBLE);

                GridLayout gl = (GridLayout) findViewById(R.id.g);
                for(int i = 0 ; i < gl.getChildCount(); i++) {
                    ((ImageView) gl.getChildAt(i)).setClickable(false);
                }
    ...
    public void tryAgain(View view){
        ...
        for(int i = 0 ; i < gl.getChildCount(); i++){
            ...
            ((ImageView) gl.getChildAt(i)).setClickable(true);
            places[i] = 2 ;
        }
        activePic = 0 ;
